...
def nacitaniTxt1(Movies, Seen):
    ...
    #### do proměnné zhlednuto. Pokud se uživatel v knihovně nenachází ostatní funkce nebudou fungovat.
    zhlednuto = []
    a = []
    ...
    if a == 'Tento uživatel se v knihovně nachází.':
        ...
        for i in range(0, len(seznam) - 1, 1):
            for j in range(0, len(zhlednuto) - 1, 1):
                if zhlednuto[j] == seznam[i]:
                    del seznam[i]
        del seznam[-1]

    ### Vrátí názvy všech filmů, které jsou v knihovně a které uživatel neviděl.
    if a == 'Tento uživatel se v knihovně nachází.':
        seznam.append(seznam[0])
        Names1 = []
        Title1 = []
        for i in range(0, len(seznam) - 1, 1):
            Names1.append(seznam[i][0])
            Title1.append(Names1[i].split(':'))

    if a == 'Tento uživatel se v knihovně nachází.':
        Title1.append(Title1[0])
        for i in range(0, len(Title1) - 1, 1):
            print('Film, který jste ještě neviděli: ',Titles[int(Title1[i][0]) - 1])
